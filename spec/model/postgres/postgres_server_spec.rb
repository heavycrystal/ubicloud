...
RSpec.describe PostgresServer do
  ...
  describe "#before_validation" do
    it "copies intended_vm_size from the resource when nil" do
      ...
      expect(server.intended_vm_size).to eq("r8gd.large")
    end

    it "preserves an already-set intended_vm_size" do
      server = described_class.new
      server.intended_vm_size = "r8gd.large"
      allow(server).to receive(:resource).and_return(instance_double(PostgresResource, target_vm_size: "r6gd.large"))
      server.before_validation
      expect(server.intended_vm_size).to eq("r8gd.large")
    end

    it "leaves intended_vm_size nil when there is no resource" do
      server = described_class.new
      allow(server).to receive(:resource).and_return(nil)
      server.before_validation
      expect(server.intended_vm_size).to be_nil
    end
  end

  describe "#on_intended_type?" do
    it "is true when vm display_size matches intended_vm_size" do
      postgres_server.intended_vm_size = "r8gd.large"
    ...
    it "is false when vm display_size differs from intended_vm_size" do
      postgres_server.intended_vm_size = "r8gd.large"
    ...
    it "translates burstable to hobby for shared-CPU servers" do
      postgres_server.intended_vm_size = "hobby-2"
  ...
  describe "#failover_target" do
    ...
    it "returns nil if there is no fresh standby" do
      ...
      standby_server = described_class.new {
        ...
        it.intended_vm_size = "standard-4"
      }
