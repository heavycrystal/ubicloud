...
RSpec.describe PostgresResource do
  ...
  describe "#reconcile_server_intended_vm_sizes" do
    before { postgres_resource.update(target_vm_size: "r6gd.large") }

    def stub_servers(servers)
      ds = instance_double(PostgresServer.dataset.class)
      allow(postgres_resource).to receive(:servers_dataset).and_return(ds)
      allow(ds).to receive(:eager).with(:vm).and_return(ds)
      allow(ds).to receive(:all).and_return(servers)
    end

    it "skips servers whose intended_vm_size already matches the resource target" do
      server = instance_double(PostgresServer, intended_vm_size: "r6gd.large")
      stub_servers([server])
      expect(server).not_to receive(:update)
      postgres_resource.reconcile_server_intended_vm_sizes
    end

    it "bumps server intended_vm_size when vm display_size matches the new resource target" do
      vm = instance_double(Vm, display_size: "r6gd.large")
      server = instance_double(PostgresServer, intended_vm_size: "r8gd.large", vm:)
      stub_servers([server])
      expect(server).to receive(:update).with(intended_vm_size: "r6gd.large")
      postgres_resource.reconcile_server_intended_vm_sizes
    end
  ...
  describe "#update_target_sizes_with_replicas" do
    it "updates self and each read replica, reconciling servers on both" do
      ...
      expect(postgres_resource).to receive(:reconcile_server_intended_vm_sizes)
      expect(replica).to receive(:reconcile_server_intended_vm_sizes)
