...
class PostgresResource < Sequel::Model
  ...
  # When target_vm_size changes, set the intended_vm_size of any server whose
  # VM is already on the new target so it isn't recycled.
  def reconcile_server_intended_vm_sizes
    servers_dataset.eager(:vm).all.each do |s|
      next if s.intended_vm_size == target_vm_size
      s.update(intended_vm_size: target_vm_size) if s.vm.display_size == target_vm_size
    end
  end
