...
class PostgresServer < Sequel::Model
  ...
  def before_validation
    self.intended_vm_size ||= resource&.target_vm_size
    super
  end
  ...
  def on_intended_type?
    vm.display_size.gsub("burstable", "hobby") == intended_vm_size
  end
  ...
  def needs_recycling?
    ...
    size_mismatch = intended_vm_size != resource.target_vm_size || storage_size_gib != resource.target_storage_size_gib
  ...
  def failover_target(mode: "unplanned")
    ...
    target = candidates
      ...
      .max_by {
        ...
        rank_score = Option.family_rank(it[:server].vm.family)
